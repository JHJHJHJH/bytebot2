...
For the standard Anthropic, OpenAI, or Gemini setup, you do not need LiteLLM or provider keys in `.env`. Deploy Bytebot normally and register provider keys from the UI at `/models`.

## Why Use LiteLLM?

<Cards>
  <Card title="100+ LLM Providers">
...
<Steps>
  ...
  ### Use Docker Compose with Proxy
    ...
    # Optional: set provider keys for the LiteLLM proxy.
    # Standard Bytebot deployments use /models instead.
    cat > docker/.env << EOF
    # Add any combination of these proxy keys
    ...
    - Makes proxy-configured models available through the proxy
  </Step>
...
1. **bytebot-llm-proxy Service**: A LiteLLM instance running in Docker that:
   ...
   - Inherits proxy provider keys from environment variables when proxy mode is used
...
<Cards>
  <Card title="Supported Models" href="https://docs.litellm.ai/docs/providers">
    Full list of 100+ providers
  </Card>
  <Card title="LiteLLM Proxy Docs" href="https://docs.litellm.ai/docs/simple_proxy">
    Official LiteLLM proxy server documentation
  </Card>
  <Card title="LiteLLM Docs" href="https://docs.litellm.ai">
