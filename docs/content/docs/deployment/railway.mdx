...
> **TL;DR -** Click the button below, deploy with the defaults, then add your AI provider API key from the Bytebot UI at `/models`. Your personal Bytebot instance will be live in ~2 minutes.
...
| ---------------- | -------------------------------------------------------------------- | ---- | -------- | ------------------------------------ |
| **bytebot-ui**   | `ghcr.io/jhjhjhjh/bytebot-ui:edge`                                 | 9992 | **Yes**  | Next.js web UI rendered to the world |
| **bytebot-agent**| `ghcr.io/jhjhjhjh/bytebot-agent:edge`                              | 9991 | No       | Task orchestration & LLM calls       |
| **bytebot-desktop**| `ghcr.io/jhjhjhjh/bytebot-desktop:edge`                          | 9990 | No       | Containerised Ubuntu + XFCE desktop  |
...
<Steps>
  ...
  ### 2. Configure Environment
    Keep the default service variables as is. LLM provider API keys are configured after deployment from the Bytebot UI, not as Railway environment variables.
  </Step>
  <Step>
  ...
  ### 4. Launch Bytebot
    When the build logs show *"bytebot-ui: ready"*, click the generated URL (e.g. `https://bytebot-ui-prod.up.railway.app`). You should see the task interface.
    _Tip: You can tail logs for each service from the Railway dashboard._
  </Step>
  <Step>

  ### 5. Add a Model Provider
    Open the generated UI URL, go to `/models`, add an Anthropic, OpenAI, or Gemini API key, and test the connection before creating your first task.
  </Step>
</Steps>
...
| No models available or provider returns `401`/`403` | Missing/invalid API key | Add or test your provider key from the Bytebot UI at `/models` |
