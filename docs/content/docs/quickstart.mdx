...
<Tabs items={["Railway (Easiest)","Docker Compose","Kubernetes/Helm","Desktop Only"]}>
  <Tab value="Railway (Easiest)">
...
<Steps>
  ...
  ### Deploy the Template
    Keep the default service settings and start the deployment.
  </Step>
  <Step>

  ### Launch &amp; Add a Model Key
    Hit **Deploy**. Railway will build the stack, wire the services together via private networking and output a public URL for the UI. Open the UI, go to `/models`, and add an Anthropic, OpenAI, or Gemini API key before creating your first task.
  </Step>
</Steps>
...
- Docker Compose
- 4GB+ RAM available
- An AI API key from one of these providers to register in `/models` after deployment:
...
Navigate to [http://localhost:9992](http://localhost:9992) to access the Bytebot UI. Open [http://localhost:9992/models](http://localhost:9992/models), add your Anthropic, OpenAI, or Gemini API key, and test the connection.
...
<Cards>
  ...
  <Card title="Virtual Desktop">
    - Full Ubuntu environment
    - Browser, office tools
    - File system access
    - Application support
  </Card>
  <Card title="Task Interface">
...
<Cards>
  <Card title="Take a Screenshot">
    "Take a screenshot of the desktop"
  </Card>
  <Card title="Open Browser">
...
<Cards>
  ...
  <Card title="Multi-Step Workflow">
...
| **Models UI**    | [http://localhost:9992/models](http://localhost:9992/models)             | Add and test LLM provider API keys            |
...
<Accordions type="multiple">
  ...
  <Accordion title="Agent errors or no response">
    Check that a model provider key is configured:
    ```bash 
    docker-compose -f docker/docker-compose.yml logs bytebot-agent 
    ```
    Open [http://localhost:9992/models](http://localhost:9992/models), add or test a valid API key from Anthropic, OpenAI, or Google, and ensure your provider account has available credits or quota.
  </Accordion>
</Accordions>
...
<Cards>
  ...
  <Card title="Takeover Mode" href="/guides/takeover-mode">
  ...
  <Card title="API Integration" href="/api-reference/introduction">
...
<Accordions type="multiple">
  <Accordion title="AI Provider Settings">
    LLM provider API keys are configured from the Bytebot UI at `/models`, not in `docker/.env`. The Docker Compose deployment persists those keys in the `bytebot_agent_config` volume so they survive container restarts.
  </Accordion>
